...
namespace kdTree
{
    public class KDTree
    {
        ...
        public static Node BuildTree(List<PointList> pointsX, List<PointList> pointsY, int depth)
        {
            Node node = null;
            int pivot = 0;
            ...
            if (pointsX.Count == 1)
            {
                return new Node{x = pointsX.First().x, y = pointsX.First().y, before = null, left = null, right = null, depth = depth, type = "point"};
            }
            else if (pointsX.Count > 1)
            {
                node = new Node();
                if (depth % 2 == 0)
                {

                    List<PointList> leftSideX = new List<PointList>();
                    List<PointList> rightSideX = new List<PointList>();
                    List<PointList> leftSideY = new List<PointList>();
                    List<PointList> rightSideY = new List<PointList>();

                    leftSideX = pointsX.Take(pointsX.Count / 2).ToList();
                    rightSideX = pointsX.Skip(pointsX.Count / 2).ToList();

                    if (pointsX.Count % 2 == 0)
                        pivot = leftSideX.Last().x;
                    else
                        pivot = rightSideX.First().x;


                    foreach (PointList point in pointsY)
                    {
                        if (point.x <= pivot)
                        {
                            leftSideY.Add(point);
                        }
                        else if (point.x > pivot)
                        {
                            rightSideY.Add(point);
                        }
                    }
                ...
                else if (depth % 2 != 0)
                {
                    ...
                    leftSideY = pointsY.Take(pointsY.Count / 2).ToList();
                    rightSideY = pointsY.Skip(pointsY.Count / 2).ToList();
                    if (pointsY.Count % 2 == 0)
                        pivot = leftSideY.Last().y;
                    else
                        pivot = rightSideY.First().y;

                    foreach (PointList point in pointsX)
                    {
                        if (point.y <= pivot)
                        {
                            leftSideX.Add(point);
                        }
                        else if (point.y > pivot)
                        {
                            rightSideX.Add(point);
                        }
                    }

                    //pivot = Pivot(leftSide, depth);
        ...
        public static int Pivot(PointList[] points, int depth)
        {
        ...
        static void Main(string[] args)
        {
            ...
            List<PointList> userPoints = new List<PointList>();
            List<PointList> userClonePoints = new List<PointList>();
            for (int i = 0; i < n; i++)
            {
                ...
                userPoints.Add(new PointList {x = x, y = y});
            }
            ...
            userPoints.OrderBy(p => p.x);
            userClonePoints = userPoints.OrderBy(p => p.y).ToList();


            tree = BuildTree(userPoints, userClonePoints, 0);
            tree.Print();
            //WriteThatTree(tree);
